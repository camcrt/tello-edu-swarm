...
class TelloNode():
    def __init__(self, node):
        # ROS node
        print("on entre dans init")
        self.node = node

        # Declare 
        self.node.declare_parameter('connect_timeout', 10.0)
        self.node.declare_parameter('tello_ip', '192.168.43.38')
...
def main(args=None):
    ...
    print ("creation du noeud tello")

    node = rclpy.create_node('tello')
    print ("noeud tello cree")
    tello_node = TelloNode(node)

    rclpy.spin(node)

    tello_node.cb_shutdown()
    node.destroy_node()
    rclpy.shutdown()
